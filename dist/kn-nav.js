...
function checkKnHttp() {
	...
	const cur = String(KnHttp.VERSION || "0").split(".").map((v) => parseInt(v) || 0), min = KNHTTP_MIN_VERSION.split(".").map((v) => parseInt(v) || 0);
...
function start(url, opt, action, link, state) {
	cancel();
	const bust = opt.cacheBust ? "_kn=" + Date.now() : null, requestUrl = bust ? addQueryParam(stripHash(url), bust) : stripHash(url), client = opt.client ?? KnHttp, req = resolveValue(opt.request);
